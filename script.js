...
function buildReferenceSlidesDocument(slides) {
  ...
  const pdfSection = buildReferencePdfSection();
    ...
    <html lang="en">
      <head>
        ...
        <style>
          ...
          .reference-slide--pdf .reference-body {
            display: flex;
            flex-direction: column;
            gap: 20px;
          }
          .reference-pdf {
            ...
            border-radius: 10px;
            overflow: hidden;
            box-shadow: inset 0 0 0 1px rgba(52, 64, 235, 0.06);
          }
          ...
          .reference-pdf__fallback a:hover,
          .reference-pdf__fallback a:focus {
...
function buildReferencePdfSection() {
  const externalUrl = escapeHtml(REFERENCE_PDF_URL);
  const localUrl = escapeHtml(LOCAL_REFERENCE_PDF_PATH);

  return `<article class="reference-slide reference-slide--pdf" id="reference-slide-pdf">
      <header class="reference-header">
        <span class="reference-number">Resource</span>
        <h2 class="reference-title">Pre-Exercise Screen (PDF)</h2>
      </header>
      <div class="reference-body">
        <p>Review the completed pre-exercise screening form directly below. The document is embedded to avoid pop-up blockers.</p>
        <div class="reference-pdf">
          <iframe class="reference-pdf__frame" src="${externalUrl}" title="Pre-exercise Screen PDF" loading="lazy"></iframe>
        </div>
        <p class="reference-pdf__fallback">If the embedded PDF does not load, <a href="${localUrl}" target="_blank" rel="noopener">open the local copy in a new tab</a>.</p>
      </div>
    </article>`;
}
