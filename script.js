...
function showCompletionMessage() {
  ...
  summary.classList.add('completion-message__text');
  summary.textContent =
  ...
  downloadButton.addEventListener('click', handleReportDownloadClick);
...
async function handleReportDownloadClick(event) {
  ...
  try {
    ...
    alert(
      'We were unable to generate the PDF report. Please check your internet connection and try again.'
    );
  } finally {
...
async function downloadReportPdf() {
  ...
  try {
    const html2pdfInstance = await ensureHtml2PdfLoaded();
    const fileNameParts = ['Case_Study_Report'];
    if (studentDetails.firstName) {
      fileNameParts.push(studentDetails.firstName.trim());
    }
    if (studentDetails.lastName) {
      fileNameParts.push(studentDetails.lastName.trim());
    }
    const filename = `${fileNameParts
      .join('_')
      .replace(/[^\w-]+/g, '_')
      .replace(/_+/g, '_')
      .replace(/^_+|_+$/g, '') || 'Case_Study_Report'}.pdf`;
...
function buildReportElement() {
  ...
  title.textContent = 'HESC3504 Metabolic Syndrome Case Study Report';
  report.appendChild(title);

  const generated = document.createElement('p');
  generated.classList.add('report-export__generated');
  generated.textContent = `Report generated on ${new Date().toLocaleString()}`;
  report.appendChild(generated);

  const details = document.createElement('dl');
  details.classList.add('report-export__details');
  appendDetail(details, 'First name', studentDetails.firstName || 'Not provided');
  appendDetail(details, 'Last name', studentDetails.lastName || 'Not provided');
  appendDetail(details, 'Student email', studentDetails.studentEmail || 'Not provided');
  appendDetail(details, 'Session ID', sessionId);
  report.appendChild(details);

  SLIDE_DECK_CONTENT.forEach((slide) => {
    const section = document.createElement('section');
    section.classList.add('report-export__section');

    const heading = document.createElement('h2');
    heading.textContent = `Slide ${slide.slideNumber}: ${slide.title}`;
    section.appendChild(heading);

    appendSlideContentForReport(section, slide);

    if (shouldIncludeResponse(slide.type)) {
      const responseEntry = responseLog.find(
        (entry) => entry.slideNumber === slide.slideNumber
      );
      const responseBlock = createResponseBlock(slide, responseEntry);
      section.appendChild(responseBlock);
    }
...
function appendDetail(container, term, description) {
  ...
  dd.textContent = description || 'Not provided';
  container.appendChild(dt);
  container.appendChild(dd);
}

function appendSlideContentForReport(section, slide) {
  if (slide.type === 'introForm') {
    appendInfoContent(section, slide.content || '');
    if (Array.isArray(slide.studentFields) && slide.studentFields.length) {
      const list = document.createElement('ul');
  ...
  if (slide.question) {
    ...
    question.innerHTML = formatTextBlock(slide.question);
  ...
  if (slide.type === 'textInput') {
    if (Array.isArray(slide.labels)) {
      ...
      section.appendChild(labelsList);
    } else if (slide.label) {
      const labelNote = document.createElement('p');
      labelNote.textContent = slide.label;
      section.appendChild(labelNote);
    }
  }

  if (slide.type === 'dropdown_multi' && Array.isArray(slide.questions)) {
    slide.questions.forEach((question) => {
      const container = document.createElement('div');
      container.classList.add('report-export__subsection');
      const prompt = document.createElement('p');
      prompt.innerHTML = formatTextBlock(question.q || '');
      container.appendChild(prompt);
  ...
  if (slide.type === 'form' && Array.isArray(slide.fields)) {
    const list = document.createElement('ul');
    slide.fields.forEach((field) => {
      const li = document.createElement('li');
      li.innerHTML = `<strong>${escapeHtml(field.label)}:</strong> ${escapeHtml(
        field.criteria || ''
      )}`;
      list.appendChild(li);
    });
    ...
    totalNote.textContent = 'Total Risk Score entry required.';
    section.appendChild(totalNote);
  }

  if (slide.type === 'multiQuestion' && Array.isArray(slide.questions)) {
    slide.questions.forEach((question) => {
      const container = document.createElement('div');
      container.classList.add('report-export__subsection');
      const prompt = document.createElement('p');
      prompt.innerHTML = formatTextBlock(question.prompt || question.q || '');
      container.appendChild(prompt);
      if (Array.isArray(question.options)) {
        const list = document.createElement('ul');
        question.options.forEach((option) => {
...
function shouldIncludeResponse(slideType) {
  return [
    'introForm',
    'checkbox',
    'radio',
...
function createResponseBlock(slide, responseEntry) {
  const container = document.createElement('div');
  container.classList.add('report-export__response');

  const heading = document.createElement('h3');
  heading.textContent = 'Your Response';
  container.appendChild(heading);

  if (slide.type === 'introForm') {
    appendIntroFormResponses(container);
    return container;
  }

  const response = responseEntry ? responseEntry.response : null;

  if (isResponseEmptyForSlide(slide.type, response)) {
    const empty = document.createElement('p');
    empty.textContent = 'No response recorded.';
  ...
  switch (slide.type) {
    ...
    case 'textInput': {
      ...
      responses.forEach((value, idx) => {
        ...
        if (responses.length > 1) {
          paragraph.innerHTML = `<strong>Response ${idx + 1}:</strong> ${safeValue}`;
        } else {
          paragraph.innerHTML = safeValue;
        }
    ...
    case 'dropdown_multi': {
      response.forEach((item, idx) => {
        const wrapper = document.createElement('div');
        wrapper.classList.add('report-export__subsection');
        const label = document.createElement('p');
        label.innerHTML = `<strong>Question ${idx + 1}:</strong> ${escapeHtml(
          item.question || ''
        )}`;
    ...
    case 'form': {
      ...
      if (response.total) {
        ...
        total.innerHTML = `<strong>Total Risk Score:</strong> ${escapeHtml(
          response.total
        )}`;
      ...
      response.forEach((item, idx) => {
        ...
        if (Array.isArray(item.answer)) {
          if (!item.answer.length) {
            ...
            none.textContent = 'No response recorded.';
...
function appendIntroFormResponses(container) {
  const list = document.createElement('dl');
  list.classList.add('report-export__details');
  appendDetail(list, 'First name', studentDetails.firstName || 'Not provided');
  appendDetail(list, 'Last name', studentDetails.lastName || 'Not provided');
  appendDetail(list, 'Student email', studentDetails.studentEmail || 'Not provided');
  container.appendChild(list);
}

function isResponseEmptyForSlide(slideType, response) {
  switch (slideType) {
    case 'checkbox':
    ...
    case 'textInput': {
      if (Array.isArray(response)) {
        return response.every((value) => !value || !value.trim());
      }
    ...
    case 'dropdown_multi': {
      ...
      return response.every((item) => !item || !item.answer || !item.answer.trim());
    }
    case 'form': {
      if (!response || typeof response !== 'object') {
        return true;
      }
      const hasFieldAnswer = Array.isArray(response.fields)
        ? response.fields.some((field) => field && field.answer && field.answer.trim())
        : false;
      const hasTotal = response.total && response.total.trim();
      return !hasFieldAnswer && !hasTotal;
    }
    case 'multiQuestion': {
      if (!Array.isArray(response)) {
        return true;
      ...
      return response.every((item) => {
        ...
        return !item.answer || !String(item.answer).trim();
      });
    }
    default:
      return true;
  ...
  html2pdfLoaderPromise = new Promise((resolve, reject) => {
    ...
    script.onload = () => {
      if (typeof window.html2pdf === 'function') {
        ...
        reject(new Error('html2pdf library did not expose an initializer.'));
      }
    };
